  ...
  async function refreshNetworkConfig() {
    try {
      ...
      const hasDifferentNetworkConfig =
        networkConfig &&
        (network.chainId !== networkConfig.moa_chain_id ||
          network.roundDuration !== networkConfig.moa_round_duration);

      if (hasDifferentNetworkConfig) {
        dispatch(
          updateNetworkConfig({
            chainId: networkConfig.moa_chain_id,
            roundDuration: networkConfig.moa_round_duration
          })
        );
      }
    } catch (err) {
